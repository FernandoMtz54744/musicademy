...
export default function Evaluation({evaluacion, handleChange, handleSubmit, data, userAnswer, isAnswering, handleBack}) {
  ...
  return (
      <form action="" className='evaluacion-form'>
            ...
            <div className='opciones-container'>
              {pregunta.opciones.map((opcion, j)=>(
                <label htmlFor={`${i}-${j}`} 
                  ...
                  ${isAnswering?"":Number(data[`pregunta_${i}`]) === j?(userAnswer[i]?"correct":"incorrect"):pregunta.correcta === j?"correct":""}`} 
                  key={j} >
        ...
        {isAnswering?(
          <input type="button" value="Terminar Evaluación" onClick={handleSubmit} className='button-finish-evaluation' id='finish-evaluation'/>
        ):(
